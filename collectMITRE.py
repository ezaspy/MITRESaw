...
def main():
    def doCollect(eachid):
        ...
        technique = re.sub(r"[\S\s]+\s{2,}", r"<span class=\"h5 card-title\">Technique: ", str(re.findall(regex, str(BeautifulSoup(urllib.request.urlopen("https://attack.mitre.org/techniques/{}/".format(eachid)).read(),"html.parser").find_all()))[0]))
        description = "<span class=\"h5 card-title\">Description: "+str(re.sub(r"</p><p>", r" ", str(re.sub(r"\<span\s.*\<\/span>", r"", str(re.findall(r"<div class=\"description-body\">[\S\s]{4}(.*)</p>", str(BeautifulSoup(urllib.request.urlopen("https://attack.mitre.org/techniques/{}/".format(eachid)).read(),"html.parser").find_all()))[1]))).replace(". .",".").replace(".\".",".").replace("<code>","").replace("</code>","")))+"."
        techniquecard, cards = str(technique+description+str(str((BeautifulSoup(urllib.request.urlopen("https://attack.mitre.org/techniques/{}/".format(eachid)).read(),"html.parser").find_all())).replace("\\n","\n").replace("\n","\\n").split("<div class=\"col-md-4\">\\n<div class=\"card\">\\n<div class=\"card-body\">\\n<div class=\"card-data\" id=\"card-id\">")[1]).split("</div>\\n</div>\\n</div>\\n<div class=\"text-center pt-2 version-button live\">\\n<div class=\"live\">")[0]).replace("\\\"","\""), []
        try:
    ...
    for techniques in techniquestable.split("<tr class=\"technique\">")[1:]:
        ...
        for eachsub in subtechniques.split("<a href=\"")[4:]:
            subid, techniquelist = str(re.sub(r"/techniques/T\d+/\d+\">( )", r"\1", str(eachsub.replace("<tr class=\"sub technique\">","").strip()))).strip(), []
            ...
            for eachtechnique in collectedtechniques:
                for eachtactic in str(eachtechnique.split("'Tactic: ")[1]).split("'")[0].split(","):
                    for eachplatform in str(eachtechnique.split("'Platforms: ")[1]).split("'")[0].split(","):
                        ...
                        entries[key] = value
    if verbose:
        print("    -> Collection of MITRE ATT&CK Techniques completed.\n")
    else:
        pass
    with open("collectedMITRE.csv", "a") as mitrecsv:
        mitrecsv.write("technique,description,subid,id,tactic,platform,permissions_required,effective_permissions,data_sources,defense_bypassed,version,created,last_modified,threat_actor\n")
        for k, v in entries.items():
            ...
            for eachvalue in v.replace("'","").replace("[","").replace("]","").strip().split(", "):
                technique = re.findall(r"^Technique: ([^\,]+)\,", details)[0]
                description = re.sub(r"(\w);(\w)", r"\1 \2", str(re.sub(r"T[\d\.]{4,8}\">", r" ", str(re.findall(r"Description: ([\S\s]+)\.\,", details.replace("..",".").replace(". ,",".,").replace(".\",",".,").replace("&gt;",">").replace("&lt;","<").replace("<p>","").replace("</p>","").replace("<ul><li>","(*)").replace("</li><li>","(*)").replace("</li></ul>","(*)").replace("</a>",""))[0]))))
                techniqueid = re.findall(r"ID: (T[^\,]+)\,", details)[0]
                if "." in techniqueid:
                    row = "{},{},{}".format(technique, description, techniqueid)
                else:
                    row = "{},{},{}.000".format(technique, description, techniqueid)
