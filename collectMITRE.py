        ...
        for k, v in entries.items():
            ...
            for eachvalue in str(str(v).replace("\\'","'").replace("[","").replace("]","").strip()).split(", "):
                ...
                if ",G0" not in row:
                    ...
                    row = row.replace(" Through "," through ").replace(" Over "," over ").replace(" In "," in ").replace(" Or "," or ").replace(" From "," from ").replace(" To "," to ").replace(" For "," for ").replace(" The "," the ").replace(" And "," and ").replace("Rundll,","Rundll32,") # adjust MITRE techniques names in elrond to reflect these changes...
                    mitrecsv.write(row)
                else:
                    pass
